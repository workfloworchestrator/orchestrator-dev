...
`workflows`, and `db` modules. The application is started with:
...
- In both backends set a federation key on the GraphQL types to join
  - WFO: [`graphql_utils/federation.py`](graphql_utils/federation.py)
...
This example orchestrator demonstrates the local table approach. A `CustomerTable` is added to the database and populated through a migration with example customers. The default GraphQL resolvers are overridden to read from the database instead of returning the static default.

The implementation involves the following files:

1. **[`db/models.py`](db/models.py)** — defines the `CustomerTable` model with `customer_id`, `fullname`, and `shortcode` columns.
2. **[`db/__init__.py`](db/__init__.py)** — registers the model with `ALL_DB_MODELS` so it is picked up by Alembic.
3. **[`graphql_utils/resolvers.py`](graphql_utils/resolvers.py)** — overrides the `customer` field on `SubscriptionInterface` and `ProcessType` using `override_class`. This is the recommended approach from orchestrator-core — it ensures all autogenerated subscription types inherit the override.
4. **[`wsgi.py`](wsgi.py)** — registers the custom subscription interface with `app.register_graphql()`.
5. **[`migrations/versions/schema/2026-06-16_b3a1c7e9f012_add_customer_table.py`](migrations/versions/schema/2026-06-16_b3a1c7e9f012_add_customer_table.py)** — creates the table and seeds example customers.
6. **[`workflows/shared.py`](workflows/shared.py)** — contains the `customer_selector()` function that queries the `CustomerTable` and returns a `Choice` type, which the orchestrator frontend renders as a customer dropdown in create workflow forms. The selected customer ID is then passed to `from_product_id(customer_id=customer_id, ...)` when constructing the subscription.
...
For more details on extending the GraphQL schema, see the WFO documentation on GraphQL[^11].
